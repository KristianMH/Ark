...
// for bug-fixing. some bools / values from pipe-regs may be missing.
...
void interp_if(){
  ...
  if_id.inst = addr;
  if (id_ex.mem_read && (id_ex.rs == GET_RS(addr) || id_ex.rt == GET_RT(addr))) {
    //insert nop
    if_id.inst = 0;
    return;
  }
...
int alu(){
  ...
  case FUNCT_NOR:
    ...
    return 0;
  case FUNCT_SLL:
    ex_mem.alu_res = second << id_ex.shamt;
    return 0;
  case FUNCT_SLT:
...
int detectexHazard(){
  // returns nonzero value of ex-hazards where detected
  int returnvalue = 0;
  if (ex_mem.reg_write && ex_mem.reg_dst == 0) {
    return returnvalue;
  }
  if (ex_mem.reg_write && ex_mem.reg_dst == id_ex.rs) {
    id_ex.rs_value = ex_mem.alu_res;
    returnvalue ++;
  }
  if (ex_mem.reg_write && ex_mem.reg_dst == id_ex.rt) {
...
int detectmemHazard(){
  // returns nonzero value of mem-hazards where detected
  int returnvalue = 0;
  if (!mem_wb.reg_write) {
    return returnvalue;
  }
  ...
  if (mem_wb.reg_dst == id_ex.rs &&
      !(ex_mem.reg_write && ex_mem.reg_dst != 0 && ex_mem.reg_dst != id_ex.rs)) {
    returnvalue++;
    if (mem_wb.mem_to_reg) {
  ...
  if (mem_wb.reg_dst == id_ex.rt &&
      !(ex_mem.reg_write && ex_mem.reg_dst != 0 && ex_mem.reg_dst != id_ex.rt)) {
    returnvalue++;
    if (mem_wb.mem_to_reg) {
